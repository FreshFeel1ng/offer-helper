...
```
$ offerhelper search "AI Agent" --city 广州 --welfare "双休,五险一金"
$ offerhelper scan-apply --max-pages 5
$ offerhelper conversations
```
...
| 传统流程 | offer-helper |
|----------|---------------------|
| 打开网页逐个搜索 | `offerhelper search` 一行搜全国 |
...
```bash
git clone https://github.com/FreshFeel1ng/offer-helper.git
cd offer-helper
pip install -e .              # 含 CLI 入口 offerhelper
...
本项目使用 PostgreSQL 存储数据，需要先创建数据库并建表：

```bash
# 1. 安装 PostgreSQL 并启动服务

# 2. 创建数据库
psql -U postgres -c "CREATE DATABASE offer_helper;"

# 3. 配置连接信息
cp .env.example .env
# 编辑 .env 填入你的 PG_HOST / PG_USER / PG_PASSWORD / PG_DATABASE

# 4. 一键建表
psql -U postgres -d offer_helper -f schema.sql
```

> 共 9 张表：投递记录、HR 会话、聊天消息、设置、每日统计、候选池、面试会话、线下面试地点、Token 用量统计。
...
```bash
$ offerhelper search "AI Agent" --city 广州 --welfare "双休,五险一金"
$ offerhelper scan-apply --max-pages 5     # 翻 5 页扫描投递 + 公司去重 + HR 过滤
$ offerhelper apply-batch                  # 投递所有待投递
$ offerhelper status                       # 看浏览器+今日统计
```
...
安装后即获 `offerhelper` 命令，stdout 仅输出 JSON，AI Agent 友好。

### 18 条命令

| 命令 | 说明 | 示例 |
|------|------|------|
| `offerhelper search` | 搜索岗位 | `offerhelper search "AI Agent" --city 广州 --welfare "双休,五险一金"` |
| `offerhelper scan` | 翻页扫描（不投递） | `offerhelper scan --max-pages 3` |
| `offerhelper scan-apply` | 翻页扫描投递（公司去重 + HR 过滤） | `offerhelper scan-apply --max-pages 5` |
| `offerhelper status` | 浏览器状态 + 今日统计 | `offerhelper status` |
| `offerhelper stats` | 投递转化漏斗 | `offerhelper stats` |
| `offerhelper jobs` | 岗位列表 | `offerhelper jobs --status pending --limit 10` |
| `offerhelper apply` | 投递单个 | `offerhelper apply <job_url>` |
| `offerhelper apply-batch` | 批量投递待投递 | `offerhelper apply-batch` |
| `offerhelper conversations` | HR 会话列表 | `offerhelper conversations` |
| `offerhelper chat` | 查看聊天记录 | `offerhelper chat 1` |
| `offerhelper send` | 手动发消息 | `offerhelper send 1 --msg "你好"` |
| `offerhelper analyze` | AI 分析岗位匹配度 | `offerhelper analyze <job_url> --title "AI开发"` |
| `offerhelper shortlist` | 候选池增删查 | `offerhelper shortlist list` |
| `offerhelper login` | 触发扫码登录 | `offerhelper login` |
| `offerhelper schema` | 输出工具描述 JSON | `offerhelper schema` |
| `offerhelper doctor` | 环境诊断 | `offerhelper doctor` |
| `offerhelper version` | 版本号 | `offerhelper version` |
| `offerhelper server` | 管理后台服务 | `offerhelper server --start` |
...
# 3. 搜索岗位
$ offerhelper search "Golang" --city 北京
...
│   ├── automation.py         # 自动化投递 + 聊天 + 发简历/微信
│   ├── company.py            # 公司画像聚合
│   ├── firefox.py            # BOSS 搜索 + 详情 XHR + 福利筛选
│   ├── geo.py                # 城市/区/规模 BOSS 编码映射
│   ├── replier.py            # AI 回复 + 招呼语 + 简历优化上下文
│   ├── state.py              # PostgreSQL 数据持久化
│   ├── scraper.py            # 通用抓取工具
│   ├── rag.py                # JD 历史 RAG 检索
...
│   └── static/
│       └── dashboard.html    # Web 前端 (单文件 SPA)
├── offerhelper_cli/          # CLI (18 命令)
...
│dashboard │◄────────────►│  boss/app.py   │◄────────────►│  zhipin.com   │
│  .html   │  HTTP/REST   │               │  Firefox     │               │
└──────────┘              │  automation   │              └──────────────┘
                          │  replier ─────────────────►│  AI API       │
                          │  state ─────► PostgreSQL   └──────────────┘
                          └──────────────┘
                  offerhelper CLI ──► HTTP 客户端 ──► FastAPI
```
...
```bash
offerhelper doctor             # 一键诊断（环境/浏览器/DB/网络）
offerhelper status             # 浏览器运行状态
```
...
| 问题 | 解决 |
|------|------|
| 端口被占用 | `offerhelper server --port 8015` |
