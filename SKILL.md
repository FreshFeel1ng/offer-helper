...
```bash
offerhelper server --start --port 8010    # 启动后台服务
offerhelper doctor                         # 环境诊断
offerhelper schema                         # 获取工具清单
offerhelper status                         # 检查登录态
```

完成标准：
- `offerhelper doctor` 返回 `ok=true`
- `offerhelper schema` 返回 12 个可用命令
- `offerhelper status` 返回 `browser_running: true`
...
# 2. 查看岗位列表
offerhelper jobs --status pending --limit 20

# 3. AI 分析某个岗位匹配度
offerhelper analyze <job_url> --title "AI开发工程师"

# 4. 收藏感兴趣的岗位
offerhelper shortlist add --job-url <job_url> --title "AI开发"

# 5. 批量投递
offerhelper apply-batch
...
| `false` + HTTP 503 | 服务未启动 | 提示用户 `offerhelper server --start` |
...
def offerhelper_cmd(*args):
    result = subprocess.run(["offerhelper"] + list(args), capture_output=True, text=True)
    return json.loads(result.stdout)

# 搜索
r = offerhelper_cmd("search", "AI Agent", "--city", "北京")
if r["ok"]:
    jobs = r["data"]
    print(f"找到 {r['total']} 个岗位")

# 诊断
r = offerhelper_cmd("doctor")
...
function offerhelper(...args: string[]) {
  const stdout = execSync(`offerhelper ${args.join(' ')}`).toString();
  return JSON.parse(stdout);
}

const result = offerhelper('search', 'Golang', '--city', '广州');
...
```bash
result=$(offerhelper search "Golang" --city 北京)
...
1. 先运行 `offerhelper doctor` 检查环境
2. 若 `browser_running: false`，提示用户在浏览器登录
3. 运行 `offerhelper schema` 获取最新工具列表（不要硬编码命令）
4. 根据用户意图调用对应命令
