...
用法: cd D:\\offer-helper && python -m boss.app --port 8000
      或: cd D:\\offer-helper\\boss && python app.py --port 8000
"""
...
# 确保项目根目录在 sys.path 中，解决 boss.state / interview.xxx 等跨包导入
...
from .automation import BossAutomation
from .state import (
    add_application,
    get_application,
...
from .replier import generate_greeting, generate_greeting_ai
from .company import build_company_preview, rank_companies_by_position_count
...
async def on_startup():
    ...
    try:
        ...
        for name in junk_names:
            db.execute("DELETE FROM conversations WHERE hr_name = %s", (name,))
...
# AI 面试辅助子进程
_assistant_process: Optional[subprocess.Popen] = None
...
class SettingsUpdate(BaseModel):
    ...
    mineru_api_token: Optional[str] = None  # MinerU PDF 解析 Token
    siliconflow_api_key: Optional[str] = None  # 硅基流动 BGE-M3 embedding
    embedding_model: Optional[str] = None  # 嵌入模型名
...
async def selectors_status():
    ...
    if not automation or automation.page is None:
        raise HTTPException(status_code=503, detail="浏览器未启动")
    from boss.automation import SELECTORS
                ...
                try:
                    ...
                    if legal:
                        ...
                        if j.get("url"):
                            db = get_db()
                            db.execute(
                                "UPDATE applications SET legal_rep=%s, is_boss=%s WHERE job_url=%s",
...
async def api_companies_preview(
    ...
    from boss.automation import pick_top_hr, _hr_title_score
    ...
    if mode in ("top5", "all"):
        ...
        for i, cr in enumerate(companies_result[:limit_deep]):
            ...
            try:
                ...
                from boss.automation import pick_top_hr as _pick
...
async def optimize_resume(req: OptimizeResumeRequest):
    ...
    db = get_db()
    row = db.execute(
        "SELECT optimize_result, optimize_at FROM applications WHERE job_url=%s",
        (req.job_url,),
    ).fetchone()
    cached_result = None
    ...
    # RAG: 检索历史相似JD经验
    from boss.rag import similar_jds, build_rag_context, ensure_jd_embedding
    desc = desc.strip()
...
6. 用简体中文"""
    ...
    try:
        ...
        try:
            db.execute(
                "UPDATE applications SET optimize_result=%s, optimize_at=CURRENT_TIMESTAMP WHERE job_url=%s",
...
async def chat_suggestion(req: ChatSuggestionRequest):
    ...
    db = get_db()
    row = db.execute(
        "SELECT chat_suggestion_result, chat_suggestion_at FROM applications WHERE job_url=%s",
        (req.job_url,),
    ).fetchone()
    cached_result = None
...
5. 用简体中文"""
    ...
    try:
        ...
        try:
            db.execute(
                "UPDATE applications SET chat_suggestion_result=%s, chat_suggestion_at=CURRENT_TIMESTAMP WHERE job_url=%s",
...
def geo_cities(force: bool = False):
    """BOSS 支持的招聘城市列表。"""
    try:
        from boss.geo import get_cities
...
async def debug_auth():
    """临时调试端点：查看缓存的鉴权状态。"""
    from boss.geo import resolve_city_code
...
async def geo_districts(city: str, force: bool = False):
    ...
    try:
        from boss.geo import resolve_city_code, _parse_districts, _cache, get_districts
...
async def geo_areas(city: str):
    ...
    if not city:
        raise HTTPException(status_code=400, detail="缺少 city 参数")
    try:
        from boss.geo import resolve_city_code, _cache
...
def api_offline_locations():
    """获取按城市分组的线下面试地点列表。"""
    try:
        from boss.state import get_offline_locations_grouped
...
def llm_stats():
    from .state import get_token_stats, list_token_usage
...
def assistant_status():
    """面试助手服务是否在运行"""
    running = _assistant_process is not None and _assistant_process.poll() is None
    return {
        "running": running,
        "port": _ASSISTANT_PORT,
        "health_url": f"http://127.0.0.1:{_ASSISTANT_PORT}/api/health",
    }


@app.post("/api/assistant/start")
async def assistant_start():
    """启动面试助手子进程（uvicorn main:app --port 8001）"""
    global _assistant_process
    if _assistant_process and _assistant_process.poll() is None:
        return {"status": "already_running", "port": _ASSISTANT_PORT}
...
async def assistant_upload_resume(file: UploadFile = File(...)):
    """上传简历到面试助手"""
    from pathlib import Path as _P
    resume_dir = _P(_ASSISTANT_DIR) / "data" / "resumes"
    resume_dir.mkdir(parents=True, exist_ok=True)
    content = await file.read()
    target = resume_dir / (file.filename or "resume")
